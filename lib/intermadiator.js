...
const normalizeWindow = () => {
  return [
    Object.freeze(
      {
        type: 'window',
        payload: document.getElementsByTagName('body')[0]
      }
    )
  ]
}

const createElement = (type, parent, repeat = 1, range = "") => {
  const createGroup = (_parent) => {
    let group = []
    _parent.forEach((child, index) => {      
      for(let i = 0; i < repeat; i++){
        if(!!range){
          ...
          .map(e => {
            if(typeof e === 'string'){
              const microRange = e.split('-').map(e => +e)
              let completedMicroRange = []
              for(let i = microRange[0]; i < microRange[1] + 1; i++){
                completedMicroRange.push(i)
              }
              return completedMicroRange
            }
            return e
          })
          if(!rangeArr.flat().includes(index)) continue
        }
        const el = document.createElement(type)
        child.payload.appendChild(el)
        group.push(
          Object.freeze(
            {
              type: 'html_element',
  ...
  if(!parent.forEach){
    return createGroup(normalizeWindow())
  }
