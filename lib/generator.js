...
function doElementAssignment(node){
  ...
  const range = !!node.range.value ? node.range.value : ""
  if(node.repeat.value) node.repeat = node.repeat.value
  if(node.parentElement.value === "window"){
    return `const ${elementName}=myLib.createElement('${elementType}',document.body, ${+node.repeat},"${range}")\n$references.push({name:'${elementName}',ref:${elementName}})`
  }else{
    return `const ${elementName}=myLib.createElement('${elementType}',${node.parentElement.value},${+node.repeat},"${range}")\n$references.push({name:'${elementName}',ref:${elementName}})`
  }
}
